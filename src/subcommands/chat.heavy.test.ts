...
describe("chat heavy", () => {
  ...
  beforeAll(async () => {
    ...
    const { status } = runCommandSync(
      `node ${cliPath} load ${modelToUse} --identifier ${modelIdentifier} --yes`,
    );
    if (status !== 0) {
  ...
  it("should respond to simple prompt with specific model", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "What is 2+2?" | node ${cliPath} chat ${modelIdentifier} --prompt "Answer briefly:"`,
    );
  ...
  it("should respond to stdin input", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "What color is the sky?" | node ${cliPath} chat ${modelIdentifier}`,
    );
  ...
  it("should use custom system prompt", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "What is your role?" | node ${cliPath} chat ${modelIdentifier} --system-prompt "You are a helpful assistant."`,
    );
  ...
  it("should display stats when --stats flag is used", () => {
    const { status, stderr } = runCommandSync(
      `echo "Hi" | node ${cliPath} chat ${modelIdentifier} --stats`,
    );
  ...
  it("should combine prompt and stdin input", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "The capital of France" | node ${cliPath} chat ${modelIdentifier} --prompt "Complete this sentence:"`,
    );
  ...
  it("should work with default model when no model specified", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "Say hello" | node ${cliPath} chat --prompt "Respond with just 'hello'"`,
    );
  ...
  it("should handle mathematical questions", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "What is 15 * 7?" | node ${cliPath} chat ${modelIdentifier}`,
    );
  ...
  it("should fail gracefully with non-existent model", () => {
    const { status, stderr } = runCommandSync(
      `echo "test" | node ${cliPath} chat non-existent-model`,
    );
  ...
  it("should handle empty input gracefully", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "" | node ${cliPath} chat ${modelIdentifier} --prompt "Say OK"`,
    );
  ...
  it("should respond to coding questions", () => {
    const { status, stdout, stderr } = runCommandSync(
      `echo "Write a simple hello world in Python" | node ${cliPath} chat ${modelIdentifier}`,
    );
