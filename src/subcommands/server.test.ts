...
describe("server", () => {
  ...
  describe("server start", () => {
    ...
    it("should start server with custom port", () => {
      const { status, stderr } = runCommandSync(`node ${cliPath} server start --port 8080`);
      if (status !== 0) console.error("Server start stderr:", stderr);
      expect(status).toBe(0);
    });
    ...
    it("should start server with custom port and CORS", () => {
      const { status, stderr } = runCommandSync(`node ${cliPath} server start -p 9000 --cors`);
      if (status !== 0) console.error("Server start stderr:", stderr);
      expect(status).toBe(0);
    });
  ...
  describe("server status", () => {
    ...
    it("should output JSON format", () => {
      const { status, stdout, stderr } = runCommandSync(`node ${cliPath} server status --json`);
      if (status !== 0) console.error("Server status stderr:", stderr);
      expect(status).toBe(0);
      if (stdout.trim()) {
