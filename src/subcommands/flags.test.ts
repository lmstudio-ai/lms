...
describe("flags", () => {
  ...
  it("should list all flags when no arguments provided", () => {
    const { status, stdout } = runCommandSync(`node ${cliPath} flags --host localhost --port 1234`);
  ...
  it("should output JSON when --json flag is used with no arguments", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags --json --host localhost --port 1234`,
    );
    expect(status).toBe(0);
  ...
  it("should check specific flag status", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags test-flag --host localhost --port 1234`,
    );
    expect(status).toBe(0);
  ...
  it("should output JSON when checking specific flag with --json", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags test-flag --json --host localhost --port 1234`,
    );
    expect(status).toBe(0);
  ...
  it("should set flag to true", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags test-flag true --host localhost --port 1234`,
    );
    expect(status).toBe(0);
  ...
  it("should set flag to false", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags test-flag false --host localhost --port 1234`,
    );
    expect(status).toBe(0);
  ...
  it("should output JSON when setting flag with --json", () => {
    const { status, stdout } = runCommandSync(
      `node ${cliPath} flags test-flag true --json --host localhost --port 1234`,
    );
    expect(status).toBe(0);
    const result = JSON.parse(stdout.trim());
  ...
  it("should reject invalid boolean values", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} flags test-flag invalid --host localhost --port 1234`,
    );
