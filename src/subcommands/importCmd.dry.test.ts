...
describe("import dry run", () => {
  ...
  it("should perform dry run without actually importing", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "${testModelPath}" --dry-run --yes --user-repo "test/model"`,
    );
  ...
  it("should show what would be done with copy flag", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "${testModelPath}" --dry-run --copy --yes --user-repo "test/model"`,
    );
  ...
  it("should show what would be done with hard link flag", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "${testModelPath}" --dry-run --hard-link --yes --user-repo "test/model"`,
    );
  ...
  it("should show what would be done with symbolic link flag", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "${testModelPath}" --dry-run --symbolic-link --yes --user-repo "test/model"`,
    );
  ...
  it("should fail when multiple operation flags are specified", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "${testModelPath}" --dry-run --copy --hard-link --yes`,
    );
  ...
  it("should handle non-existent file gracefully", () => {
    const { status, stderr } = runCommandSync(
      `node ${cliPath} import "/non/existent/file.gguf" --dry-run --yes --user-repo "test/model"`,
    );
