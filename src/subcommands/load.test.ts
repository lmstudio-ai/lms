...
describe("load", () => {
  ...
  describe("load command", () => {
    it("should show help when --help flag is used", () => {
      const { status, stdout } = runCommandSync(`node ${cliPath} load --help`);
      expect(status).toBe(1);
      expect(stdout).toContain("Load a model");
    });

    it("should load model without identifier and verify with ps", () => {
      const { status, stderr } = runCommandSync(`node ${cliPath} load gemma-3-1b --yes`);
      ...
      const {
        ...
        stdout: psOutput,
        stderr: psStderr,
      } = runCommandSync(`node ${cliPath} ps`);
    ...
    it("should load model with basic flags and verify with ps", () => {
      const { status, stderr } = runCommandSync(
        `node ${cliPath} load gemma-3-1b --identifier basic-model --yes`,
      );
      if (status !== 0) console.error("Load stderr:", stderr);
      expect(status).toBe(0);
      ...
      // Unload the model
      const { status: unloadStatus, stderr: unloadStderr } = runCommandSync(
        `node ${cliPath} unload basic-model`,
      );
      if (unloadStatus !== 0) console.error("Unload stderr:", unloadStderr);
      expect(unloadStatus).toBe(0);
    });
    ...
    it("should handle advanced flags (GPU, TTL, context-length)", () => {
      const { status, stderr } = runCommandSync(
        `node ${cliPath} load gemma-3-1b --identifier advanced-model --ttl 1800 --gpu 0.8 --context-length 4096 --yes`,
      );
      if (status !== 0) console.error("Load stderr:", stderr);
      expect(status).toBe(0);

      // Cleanup
      runCommandSync(`node ${cliPath} unload advanced-model`);
    });

    it("should handle GPU options (off, max, numeric)", () => {
      // Test GPU off
      const { status: status1, stderr: stderr1 } = runCommandSync(
        `node ${cliPath} load gemma-3-1b --identifier gpu-off-model --gpu off --yes`,
      );
      if (status1 !== 0) console.error("Load stderr:", stderr1);
      expect(status1).toBe(0);
      runCommandSync(`node ${cliPath} unload gpu-off-model`);

      // Test GPU max
      const { status: status2, stderr: stderr2 } = runCommandSync(
        `node ${cliPath} load gemma-3-1b --identifier gpu-max-model --gpu max --yes`,
      );
      if (status2 !== 0) console.error("Load stderr:", stderr2);
      expect(status2).toBe(0);
      runCommandSync(`node ${cliPath} unload gpu-max-model`);
    });

    it("should handle custom identifier and verify in ps", () => {
      const { status, stderr } = runCommandSync(
        `node ${cliPath} load gemma-3-1b --identifier custom-gemma --yes`,
      );
      if (status !== 0) console.error("Load stderr:", stderr);
      expect(status).toBe(0);
      ...
      const { status: unloadStatus, stderr: unloadStderr } = runCommandSync(
        `node ${cliPath} unload custom-gemma`,
      );
      if (unloadStatus !== 0) console.error("Unload stderr:", unloadStderr);
      expect(unloadStatus).toBe(0);
    });
    ...
    it("should handle error cases gracefully", () => {
      ...
      const { status: status1, stderr: stderr1 } = runCommandSync(
        `node ${cliPath} load non-existent-model --exact`,
      );
      ...
      const { status: status2, stderr: stderr2 } = runCommandSync(
        `node ${cliPath} load non-existent-model --yes`,
      );
      expect(status2).not.toBe(0);
      expect(stderr2).toBeTruthy();

      // Exact flag without path
      const { status: status3, stderr: stderr3 } = runCommandSync(`node ${cliPath} load --exact`);
      expect(status3).not.toBe(0);
      expect(stderr3).toBeTruthy();
    });

    it("should verify ls shows downloaded models", () => {
      const { status, stdout, stderr } = runCommandSync(`node ${cliPath} ls`);
